...
(() => {
  ...
  const Cart = {
    ...
    add(item){
      ...
      const idx = list.findIndex(i => i.name === item.name && i.price === item.price);
      if(idx > -1){
  ...
  const Wishlist = {
    ...
    add(item){
      const list = this.list();
      const exists = list.some(i => i.name === item.name && i.price === item.price);
  ...
  // ----- UI notifications -----
  function notify(msg){ // simple fallback using alert; replace with custom UI later
    alert(msg);
  }

  // ----- Common page features -----
  function initHeaderActions(){
    // Add wishlist/cart icons could be enhanced to show counts
  }
  ...
  function initProductPage(){
    ...
    if(productImage){
      $$('.thumb').forEach(t => t.addEventListener('click', () => productImage.src = t.src));
    }
    ...
    if(addToCartBtn){
      addToCartBtn.addEventListener('click', () => {
        ...
        const priceText = $('#productPrice').innerText.replace('₹','').trim();
        const qty = Number($('#qty').value) || 1;
        Cart.add({ name, price: Number(priceText), qty, image: productImage ? productImage.src : '' });
    ...
    if(addWishlistBtn){
      addWishlistBtn.addEventListener('click', () => {
        const item = {
          name: addWishlistBtn.dataset.name || $('#productName').innerText,
          price: Number(addWishlistBtn.dataset.price || $('#productPrice').innerText.replace('₹','').trim() || 0),
          image: addWishlistBtn.dataset.image || (productImage ? productImage.src : '')
        };
        const added = Wishlist.add(item);
        notify(added ? 'Added to wishlist' : 'Already in wishlist');
      });
  ...
  function initCartPage(){
    ...
    if(checkoutBtn){
      checkoutBtn.addEventListener('click', () => {
        ...
        const total = Number(cart.reduce((s, i) => s + (Number(i.price) || 0) * (Number(i.qty) || 1), 0)).toFixed(2);
        const order = { id:`HC26-${Math.floor(Math.random()*900000+100000)}`, customer:user, paymentMethod, date:new Date().toLocaleDateString(), items:cart, total };
    ...
    notify(error.message);
      ...
      if(toggle && pwd){
        toggle.addEventListener('click', () => {
          if(pwd.type === 'password'){ pwd.type = 'text'; toggle.textContent = 'Hide Password' }
          else { pwd.type = 'password'; toggle.textContent = 'Show Password' }
        });
      }
    }
  }
  ...
  function initContactForm(){
    ...
    form.addEventListener('submit', (e) => {
      e.preventDefault();
      // collect fields for later backend integration
      const data = {
      ...
      // store locally (placeholder) and reset
  ...
  function initCheckoutPage(){
    ...
    function renderSummary(){
      ...
      if(!list || list.length === 0){
        summaryItems.innerHTML = '<p style="color:#666">Your cart is empty.</p>';
        summaryCount.textContent = 0;
        summaryTotal.textContent = formatPrice(0);
        summaryGrand.textContent = formatPrice(SHIPPING);
        return;
      }
      let total = 0;
      let items = 0;
      ...
      summaryCount.textContent = items;
      summaryTotal.textContent = formatPrice(total);
      summaryShipping.textContent = formatPrice(SHIPPING);
      summaryGrand.textContent = formatPrice(total + SHIPPING);
    }

    // prefill user
    const storedUser = Auth.get();
    if(storedUser){
      if(storedUser.fullName) $('#fullName').value = storedUser.fullName;
      if(storedUser.email) $('#email').value = storedUser.email;
      if(storedUser.phone) $('#phone').value = storedUser.phone;
    }
    ...
    checkoutForm.addEventListener('submit', (e) => {
      ...
      const subtotal = Number(summaryTotal.textContent) || 0;
      const shipping = Number(summaryShipping.textContent) || SHIPPING;
      const order = {
  ...
  function initOrderSuccess(){
    ...
    orderDate.textContent = lastOrder.date || '';
    orderPayment.textContent = lastOrder.paymentMethod || '';
    orderCustomer.textContent = (lastOrder.customer && (lastOrder.customer.fullName || lastOrder.customer.name)) || '';
    orderTotal.textContent = formatPrice(lastOrder.total || (lastOrder.subtotal || 0) + (lastOrder.shipping || 0));
